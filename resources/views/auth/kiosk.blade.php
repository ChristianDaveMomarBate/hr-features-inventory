        ...
        <div class="kiosk-hero text-center position-relative">
            ...
            <div class="kiosk-hero-fullscreen-brand-text">
                <span class="stock-logo">
                    <span class="stock-main">Stock</span><span class="stock-accent">Wise</span>
                </span>
                <small>PHRMDO Inventory Management System</small>
            </div>
            </div>
        </div>
            ...
            <div class="kiosk-layout">
                ...
                <div class="kiosk-left">
                    ...
                    <div class="kiosk-grid" >
                        ...
                        <div class="kiosk-card" {!! $item->stock <= 0 ? 'style="opacity:0.6;"' : '' !!} data-id="{{ $item->id }}" data-name="{{ $item->name }}" data-category="{{ $item->category }}" data-unit="{{ $item->display_unit }}" data-stock="{{ $item->stock }}" data-stock-unit="{{ $item->stock_unit ?? $item->unit }}" data-units-per-stock-unit="{{ $item->units_per_stock_unit ?? 1 }}" data-bulk-equivalent="{{ $item->bulk_equivalent }}" tabindex="0" role="button" aria-label="Add {{ $item->name }}">
                                ...
                                <div class="kiosk-card-stock {{ $item->stock <= 0 ? 'text-danger' : ($item->stock <= $item->minimum ? 'low' : '') }}">
                                    @if($item->stock <= 0) <span class="kiosk-stock-dot" style="background-color: #ef4444;"></span>
                                        Not available
                                        @else
                                ...
                                @endif
                                @if($item->stock <= 0) <button type="button" class="kiosk-add-btn disabled" style="background-color: #e2e8f0; color: #94a3b8; cursor: not-allowed;" disabled>
                    ...
                    <div class="kiosk-panel kiosk-info-panel">
                        ...
                        <div class="kiosk-panel-body">
                            <div class="kiosk-field">
                                ...
                                <input type="text" id="kioskName" name="requester_name" class="kiosk-input @error('requester_name') err @enderror" placeholder="e.g. Juan Dela Cruz" value="{{ old('requester_name') }}" autocomplete="name" required>
                            ...
                            <div class="kiosk-field">
                                ...
                                <select id="kioskDiv" name="division" class="kiosk-select @error('division') err @enderror" required>
